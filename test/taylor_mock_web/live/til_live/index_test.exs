defmodule TaylorMockWeb.Live.TilLive.IndexTest do
  ...
  test "renders list of posts", %{conn: conn} do
    ...
    assert html =~
             ~s/href="#{
               Routes.til_show_path(conn, :show, first_post.slug) <>
                 parameterized_title(first_post.title)
             }"/
    ...
    assert html =~
             ~s/href="#{
               Routes.til_show_path(conn, :show, second_post.slug) <>
                 parameterized_title(second_post.title)
             }"/
  end
